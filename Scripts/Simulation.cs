...
public class Simulation : MonoBehaviour
{
    ...
    IEnumerator Restocking(Food food)
    {
        ...
        float scaledTime = (float)(RestockingTimeInMinutes) * (float)(1.0/5); // the larger the multiplier, the longer it takes to 
        ...
        while (totalOnShelves[food.getItemDescription()] != food.getMaxFrontQuantity() && totalInBack[food.getItemDescription()] != 0)
        {
            totalOnShelves[itemName]++;
            totalInBack[itemName]--;
            updateFoodStocks();
