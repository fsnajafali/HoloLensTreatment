...
public class PostDayManager : MonoBehaviour
{
    ...
    public static int total_food_sold;
    [HideInInspector] public static decimal total_spent_on_deliveries;
    [HideInInspector] public static int money_spent_paying_employees;
    [HideInInspector] public static int front_of_house_stock;
    [HideInInspector] public static int back_of_house_stock;
    [HideInInspector] public static int num_of_deliveries_in_queue;
    ...
    void Start()
    {
        ...
        int registerUtil = (int)(registers.registerUtilization * 100);
        //print to the text on the screen
        status_text.text = ("You completed Day " + (TimeController.Day - 1) + "@@You have " + cash_value_neg_or_pos + "@              (Net change: " + Net_change_string + ")@Total Front of House Stock: "
                            + TotalFOH + "@Total Back of House Stock: " + TotalBOH + "@You sold " + total_food_sold + " items@" + Simulation.total_expired + " foods expired :(@$" + money_spent_paying_employees
                            + " spent on employees@$" + total_spent_on_deliveries + " spent on deliveries@" + TimeController.upcoming_deliveries.Count + " deliveries will arrive tomorrow.@Overall Register Utilization: "
                            + registerUtil + "%");
